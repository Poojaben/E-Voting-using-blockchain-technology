...
async function main() {
    try {
        ...
        const userExists = await wallet.get('0000111');
        if (userExists) {
        ...
        await wallet.put('0000111', x509Identity);
        console.log('Successfully enrolled client user "0000111" and imported it into the wallet');

    } catch (error) {
        console.error(`Failed to enroll client user "0000111": ${error}`);
        process.exit(1);
    }
}
