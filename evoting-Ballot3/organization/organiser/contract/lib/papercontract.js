...
class BallotPaperContract extends Contract {
    ...
    */
     ...
     */
    async cast(ctx, ballotNumber, electionNumber, castedonDate, selCandidate) {
        if( ballotNumber==''|electionNumber==''|selCandidate==''){
       ...
       // }
        ...
        if (paper.isIssued()) {
           ...
           // let mspid = ctx.clientIdentity.getMSPID();
            ...
            throw new Error('\nPaper '+ ballotNumber + ' is not valid. Current state = ' + paper.getCurrentState());
        }
    ...
    /**
      ...
      * @param {String} electionNumber election Number 
     */
    async cast_request(ctx, ballotNumber, castedonDate, selCandidate, electionNumber) {
        if(ballotNumber==''|electionNumber==''|selCandidate==''){
            throw new Error('Please provide required data, vote is not casted'); 
        }
else{
    ...
    * @param {String} electionNumber election Number 
     */
    async cast_confirm(ctx, ballotNumber, confirmDate,electionNumber) {
        if(ballotNumber==''|electionNumber==''){
        ...
        if ( ! paper.isPending()) {
            throw new Error('\n' + ballotNumber + ' is not currently in state: PENDING for tallying to occur: \n must run cast_request transaction first');
        }
        // else all good
    ...
    */
    async audit(ctx, ballotNumber, electionNumber, auditDate) {
        if(ballotNumber==''|electionNumber==''){
        ...
        if (paper.isIssued()| paper.isPending()) {
            throw new Error('\nPaper '  + ballotNumber + ' did not cast his vote');
        }
  ...
  return paper;
       ...
       //     throw new Error('\nPaper '+ ballotNumber + ' cannot be audited by ' + ctx.clientIdentity.getMSPID() + ', as it is not the authorised owning Organisation');
       // }
