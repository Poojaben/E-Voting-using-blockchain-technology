...
class QueryUtils {
    ...
    // =========================================================================================
    /**
    * Get Asset History for a ballot paper
    * @param {String} electionNumber election Number 
    * @param {String} ballotNumber  ballot number
    */
    async getAssetHistory(electionNumber, ballotNumber) {

        let ledgerKey = await this.ctx.stub.createCompositeKey(this.name, [electionNumber, ballotNumber]);
    ...
    /**
    * 
    * @param {String} assetspace the asset space (eg Organiser's assets)
    */
    async queryKeyByPartial(assetspace) {
        ...
        // "Key":"org.papernet.paperOrganiser0001"   (0002, etc)
        // "Partial":'org.papernet.paperlistOrganiser"'  (using partial key, find keys "0001", "0002" etc)
    ...
    // =========================================================================================
    /**
    * queryKeyByElection ballot paper
    * @param {String} electionNumber election number
    */
    async queryKeyByElection(electionNumber) {
        //  
        let self = this;
        if (arguments.length < 1) {
            throw new Error('Incorrect number of arguments. Expecting election Number.');
        }
        let queryString = {};
        queryString.selector = {};
        //  queryString.selector.docType = 'indexOwnerDoc';
        queryString.selector.electionNumber = electionNumber;
        // set to (eg)  '{selector:{owner:Organiser}}'
        let method = self.getQueryResultForQueryString;
        let queryResults = await method(this.ctx, self, JSON.stringify(queryString));
        return queryResults;
    }
    /**
    * queryKeyByElectionCasted ballot paper
    * @param {String} electionNumber election number
    */
    async queryKeyByElectionCasted(electionNumber) {
        //  
        let self = this;
        if (arguments.length < 1) {
            throw new Error('Incorrect number of arguments. Expecting election Number.');
        ...
        //  queryString.selector.docType = 'indexOwnerDoc';
        queryString.selector.electionNumber = electionNumber;
        queryString .selector.currentState= 3 | 4;
        // set to (eg)  '{selector:{owner:Organiser}}'
        let method = self.getQueryResultForQueryString;
        let queryResults = await method(this.ctx, self, JSON.stringify(queryString));
        return queryResults;
...
/**
    * queryCandElection ballot paper
    * @param {String} electionNumber election number
    * @param {String} selCandidate selected candidate
    */
    async queryCandElection(electionNumber,selCandidate) {
        //  
        let self = this;
        if (arguments.length < 1) {
            throw new Error('Incorrect number of arguments. Expecting election Number.');
        ...
        //  queryString.selector.docType = 'indexOwnerDoc';
       queryString.selector.electionNumber = electionNumber;
        queryString.selector.selCandidate = selCandidate;
    ...
    // Only available on state databases that support rich query (e.g. CouchDB)
    // example passed using VS Code ext: ["{\"selector\": {\"owner\": \"Organiser\"}}"]
    // =========================================================================================
    /**
    * query By AdHoc string (ballot paper)
                ...
                if (isHistory && isHistory === true) {
                    ...
                    if (res.value.is_delete) {
                        ...
                        try {
                            ...
                            // report the ballot paper states during the asset lifecycle, just for asset history reporting
                            switch (jsonRes.Value.currentState) {
                                ...
                                case 2:
                                    ...
                                    break;
                                case 3:
                                    jsonRes.Value.currentState = 'TALLYING';
                                    break;
                                case 4:
                                    jsonRes.Value.currentState = 'AUDITED';
                                    break;
