...
curl -X PUT http://admin:adminpw@localhost:5984/election/_design/all_elections -d '{ "views": {
   "all": {
    "map": "function (doc) {\n if(doc.type == \"election\"){\n  emit(doc._id, {type:doc.type,election_name:doc.election_name,election_date:doc.election_date,candidate_1:doc.candidate_1,candidate_2:doc.candidate_2,candidate_3:doc.candidate_3,rev:doc._rev});\n}\n  if(doc.type == \"student\"){\n    emit(doc._id,{type:doc.type,fname:doc.fname,branch:doc.branch,rev:doc._rev});\n  }\n  \n  \n}\n"
   }
  }}'
...
curl -X PUT http://admin:adminpw@localhost:5984/users/_design/all_users -d '{ "views": {
   "users": {
    	"map": "function (doc) {\n  emit(doc.matriculationnumber, {id:doc.id,firstname:doc.firstname,lastname:doc.lastname,matriculationnumber:doc.matriculationnumber,course:doc.course,semester:doc.semester,email:doc.email,password:doc.password,is_admin:doc.is_admin,user_type:doc.user_type,rev:doc._rev});\n}"
   }
  }}'
curl -H 'Content-Type: application/json' -X POST http://admin:adminpw@localhost:5984/users -d '{"firstname": "admin", "lastname": "admin", "matriculationnumber": "0000111", "course": "", "semester": "", "email": "admin@gmail.com", "password": "$2a$10$ZCL2q1p32pgnBl8dzhiELuVtptLx9lAZCwpHnDQFOQwiPoknepmOm", "is_admin":true, "user_type": "organizer"}'
curl -H 'Content-Type: application/json' -X POST http://admin:adminpw@localhost:5984/users -d '{"firstname": "admin", "lastname": "admin", "matriculationnumber": "0000222", "course": "", "semester": "", "email": "admin@gmail.com", "password": "$2a$10$ZCL2q1p32pgnBl8dzhiELuVtptLx9lAZCwpHnDQFOQwiPoknepmOm, "is_admin":true, "user_type": "verifier"}'
